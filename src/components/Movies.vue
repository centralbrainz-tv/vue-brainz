  ...
  <div class="mainblock">
    ...
    <div v-if="videos && videos.length > 0" class="white left-align data">
      ...
      <div v-if="count > 0" class="pages">
        <router-link v-for="n in Math.ceil(count / 20.0)" :key="n" :to="'/index/' + n + '/' + sortBy + '/' + sortDesc">{{ n }}&#32;</router-link>
      </div>
      <hr class="red hr800" />
...
export default {
  ...
  watch: {
    "$route.params.page": function () {
      const dataURL = this.$baseurl + "php-service/index/index/page/" + (this.$route.params.page ? this.$route.params.page : "1") + "/20/" + this.sortBy + "/" + this.sortDesc;
