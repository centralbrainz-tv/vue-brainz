        ...
        <div v-if="movie.title !== null && movie.title !== undefined && movie.title !== ''" lass="col-78 col-full-xs countdown-item-content">
            ...
            <div class="col-full-xs" style="height: 100%;">
              <div class="article_movie_title" style="float: left;">
                <div>
                  ...
                  <star-rating inactive-color="white" active-color="red" :increment="0.01" :rating="(movie.imdb.rating * 10.0) / 10.0" :fixed-points="2" :max-rating="10" :star-size="20" :border-widht="0" :read-only="true"></star-rating>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-if="movie.imdb.arrayPlotSummary[0].text !== ''" class="row row-sub countdown-item-details">
            <div>
              ...
              <div v-for="(summary, index) in movie.imdb.arrayPlotSummary" :key="index">
          ...
          <div v-if="movie.imdb.arraySynopsis[0].text !== '' && movie.imdb.arraySynopsis[0].text.indexOf('It looks like') === -1" class="row row-sub countdown-item-details">
            <div>
              ...
              <div v-for="(summary, index) in movie.imdb.arraySynopsis" :key="index">
                <p v-html="summary.text.indexOf('It looks like') === -1 ? summary.text : ''"></p>
                <span class="pink" v-html="summary.author"></span>
              </div>
            </div>
          </div>
          <div v-for="(credit, index) in movie.imdb.arrayFullCredit" :key="index" class="row row-sub countdown-item-details">
            <div style="width: 100%;">
              <span class="red" v-html="credit.name + ': '"></span>
              <div v-html="credit.text.indexOf('It looks like') === -1 ? credit.text : ''"></div>
            </div>
          </div>
          <div v-for="(obj, index) in [movie.imdb.technical]" :key="index" class="row row-sub countdown-item-details">
            <div>
              <span class="red" v-html="obj.name + ': '"></span>
              <div v-html="obj.text.indexOf('It looks like') === -1 ? obj.text : ''"></div>
            </div>
          </div>
          <div v-for="(obj, index) in movie.imdb.arrayReleaseInfo" :key="index" class="row row-sub countdown-item-details">
            <div>
              <span class="red" v-html="obj.name + ': '"></span>
              <div v-html="obj.text.indexOf('It looks like') === -1 ? replace(obj.text) : ''"></div>
            </div>
          </div>
          <ul v-if="movie.imdb.locations.text.length > 0 && movie.imdb.locations.text !== ''" class="row row-sub countdown-item-details" style="flex-wrap: wrap !important; list-style-type: disc; list-style-position: outside;">
            <span class="red" v-html="movie.imdb.locations.name + ': '"></span>
            <li
              v-for="(obj, index) in movie.imdb.locations.text.length > 0 && movie.imdb.locations.text !== '' ? movie.imdb.locations.text : []"
              :key="index"
              style="display: list-item;"
              v-html="obj.indexOf('It looks like') === -1 ? obj : ''"
            ></li>
          </ul>
          <ul v-if="movie.imdb.dates.text.length > 0 && movie.imdb.dates.text !== ''" class="row row-sub countdown-item-details" style="flex-wrap: wrap !important; list-style-type: disc; list-style-position: outside;">
            <span class="red" v-html="movie.imdb.dates.name + ': '"></span>
            <li v-for="(obj, index) in movie.imdb.dates.text.length > 0 && movie.imdb.dates.text !== '' ? movie.imdb.dates.text : []" :key="index" style="display: list-item;" v-html="obj.indexOf('It looks like') === -1 ? obj : ''"></li>
          </ul>
          <div v-for="(obj, index) in movie.imdb.keywords.length > 0 ? movie.imdb.keywords : movie.imdb.keywords.text && movie.imdb.keywords.text !== '' ? [movie.imdb.keywords] : []" :key="index" class="row row-sub countdown-item-details">
            <div>
              <span class="red" v-html="obj.name + ': '"></span>
              <div v-html="obj.text.indexOf('It looks like') === -1 ? obj.text : ''"></div>
            </div>
          ...
          <ul v-if="movie.imdb.arrayTaglines.length > 0 && movie.imdb.arrayTaglines[0].text !== ''" class="row row-sub countdown-item-details" style="flex-wrap: wrap !important; list-style-type: disc; list-style-position: outside;">
            ...
            <li
              v-for="(obj, index) in movie.imdb.arrayTaglines.length > 0 && movie.imdb.arrayTaglines[0].text !== '' ? movie.imdb.arrayTaglines : []"
              :key="index"
              style="display: list-item;"
              v-html="obj.text.indexOf('It looks like') === -1 ? obj.text : ''"
            ></li>
          </ul>
          <div v-for="(obj, index) in movie.imdb.arrayAwards" :key="index" class="row row-sub countdown-item-details">
            <div>
              <span class="red" v-html="obj.name + ': '"></span>
              <div v-html="obj.text.indexOf('It looks like') === -1 ? obj.text : ''"></div>
            </div>
          </div>
          <div v-for="(obj, index) in movie.imdb.arrayFAQ.length > 0 && movie.imdb.arrayFAQ[0].text.lenght > 0 ? movie.imdb.arrayFAQ : []" :key="index" class="row row-sub countdown-item-details">
          ...
          <div
            v-for="(obj, index) in movie.imdb.arrayParentalGuide.length > 0 && movie.imdb.arrayParentalGuide[0].text.length > 0 && movie.imdb.arrayParentalGuide[0].text[0] !== '' ? movie.imdb.arrayParentalGuide : []"
            :key="index"
          ...
          <ul v-if="movie.imdb.trivias.text.length > 0" class="row row-sub countdown-item-details" style="flex-wrap: wrap !important; list-style-type: disc; list-style-position: outside;">
            ...
            <li v-for="(obj, index) in movie.imdb.trivias.text.length > 0 ? movie.imdb.trivias.text : []" :key="index" style="display: list-item;" v-html="obj.indexOf('It looks like') === -1 ? obj : ''"></li>
          </ul>
          <div v-for="(obj, index) in movie.imdb.AV.text.length > 0 ? movie.imdb.AV.text : []" :key="index" class="row row-sub countdown-item-details">
            <div>
          ...
          <ul v-if="movie.imdb.CC.text.length > 0" class="row row-sub countdown-item-details" style="flex-wrap: wrap !important; list-style-type: disc; list-style-position: outside;">
            ...
            <li v-for="(obj, index) in movie.imdb.CC.text.length > 0 ? movie.imdb.CC.text : []" :key="index" style="display: list-item;" v-html="obj.indexOf('It looks like') === -1 ? obj : ''"></li>
          </ul>
          <div v-for="(obj, index) in movie.imdb.MC.text.length > 0 ? movie.imdb.MC.text : []" :key="index" class="row row-sub countdown-item-details">
            <div>
          ...
          <ul v-if="movie.imdb.quotes.text.length > 0" class="row row-sub countdown-item-details" style="flex-wrap: wrap !important; list-style-type: disc; list-style-position: outside;">
            ...
            <li v-for="(obj, index) in movie.imdb.quotes.text.length > 0 ? movie.imdb.quotes.text : []" :key="index" style="display: list-item;" v-html="obj.indexOf('It looks like') === -1 ? obj : ''"></li>
          </ul>
          <ul v-if="movie.imdb.goofs.text.length > 0" class="row row-sub countdown-item-details" style="flex-wrap: wrap !important; list-style-type: disc; list-style-position: outside;">
            ...
            <li v-for="(obj, index) in movie.imdb.goofs.text.length > 0 ? movie.imdb.goofs.text : []" :key="index" style="display: list-item;" v-html="obj.indexOf('It looks like') === -1 ? obj : ''"></li>
          </ul>
        </div>
      </div>
      <br />
...
export default {
  ...
  watch: {
    "$route.params.name": function () {
      const dataURL = this.$baseurl + "php-service/movie/" + this.$route.params.name + "/page/" + this.$route.params.page + "/100/name/1";
