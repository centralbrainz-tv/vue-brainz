  ...
  <div class="mainblock">
    ...
    <div v-if="videos && videos.length > 0" class="white left-align data">
      ...
      <div v-if="count > 0" class="pages">
        <router-link v-for="n in Math.ceil(count / 20.0)" :key="n" :to="'/year/' + mainParam + '/' + n + '/' + sortBy + '/' + sortDesc">{{ n }}&#32;</router-link>
      </div>
      <hr class="red hr800" />
...
export default {
  ...
  computed: {
    ...
    sortUrl() {
      return "/year/" + this.$route.params.year + "/" + this.$route.params.page;
    },
    sortPage() {
  ...
  watch: {
    "$route.params.year": function () {
      const dataURL = this.$baseurl + "php-service/year/" + this.$route.params.year + "/page/" + this.$route.params.page + "/20/" + this.sortBy + "/" + this.sortDesc;
