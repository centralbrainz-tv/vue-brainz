  ...
  <div class="mainblock">
    ...
    <div v-if="videos && videos.length > 0" class="white left-align data">
      ...
      <div v-if="count > 0" class="pages">
        <router-link v-for="n in Math.ceil(count / 20.0)" :key="n" :to="'/search/' + mainParam + '/' + n + '/' + sortBy + '/' + sortDesc">{{ n }}&#32;</router-link>
      </div>
      <hr class="red hr800" />
...
export default {
  ...
  computed: {
    ...
    sortUrl() {
      return "/search/" + this.$route.params.search + "/" + this.$route.params.page;
    },
    sortPage() {
  ...
  watch: {
    "$route.params.search": function () {
      const dataURL = this.$baseurl + "php-service/fulltext/" + this.$route.params.search + "/page/" + this.$route.params.page + "/20/" + this.sortBy + "/" + this.sortDesc;
