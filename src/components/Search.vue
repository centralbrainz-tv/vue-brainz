    ...
    <div class="mainblock">
        ...
        <div class="white left-align data" v-if="videos && videos.length > 0">
            ...
            <div v-if="count > 0" style="display: block; text-align: center;">
                <router-link v-for="n in Math.ceil(count / 20.0)" v-bind:key="n" :to="'/search/' + mainParam + '/' + n + '/' + sortBy + '/' + sortDesc">{{  n  }} </router-link>
            </div>
            <hr class="red hr800" />
...
export default {
  ...
  watch: {
    '$route.params.search': function (id) {
      const dataURL = this.$baseurl + 'php-service/fulltext/' +
